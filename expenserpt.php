...
<form  action="report.php" method="POST">
 ...
 <tr><td>
  ...
  ?> 
   ...
   <input type="hidden" name="cmbReport" size="12" value="Expenditure Report">
 </td>
 <td>
...
<TABLE width='100%' border='1' cellpadding='1' cellspacing='1' align='center' bordercolor="#ccCCcc" id="table2">
 ...
 @$filter2=$_REQUEST["filter2"];
  ...
  {  
   $query_count    = "SELECT * FROM `cash` where `Date` = '" . date('Y-m-d') . "' and `Type`='Expenditure'";
  }
  else if ($cmbFilter=="All")
  {  
   $query_count    = "SELECT * FROM `cash` where `Type`='Expenditure'";
  }
  else if ($cmbFilter=="Date Range")
  {  
   $query_count    = "SELECT * FROM `cash` where (`Date` between '" . $filter . "' and '" . $filter2 . "') and `Type`='Expenditure'"; 
  }
  ...
  if ($cmbFilter=="" or $cmbFilter=="Today" or empty($cmbFilter))
  {  
   $result = mysqli_query ($conn,"SELECT `ID`,`Type`,`Source`, `Date`,`Amount` FROM `cash` where `Date` = '" . date('Y-m-d') . "' and `Type`='Expenditure' order by `Date` desc LIMIT $limitvalue, $limit"); 
  }
  else if ($cmbFilter=="All")
  {
   $result = mysqli_query ($conn,"SELECT `ID`,`Type`,`Source`, `Date`,`Amount` FROM `cash` where `Type`='Expenditure' order by `Date` desc LIMIT $limitvalue, $limit");   
  }
  else if ($cmbFilter=="Date Range")
  {  
   $result = mysqli_query ($conn,"SELECT `ID`,`Type`,`Source`, `Date`,`Amount` FROM `cash` where (`Date` between '" . $filter . "' and '" . $filter2 . "') and `Type`='Expenditure' order by `Date` desc LIMIT $limitvalue, $limit"); 
  }
