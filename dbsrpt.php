...
<TABLE width='100%' border='1' cellpadding='1' cellspacing='1' align='center' bordercolor="#000000" id="table2">
 ...
 @$filter2=$_REQUEST["filter2"];
  ...
  {  
   $query_count = "SELECT * FROM `transactions` WHERE `Transaction Type` in ('Deposit','Charges') and (`Date` = '" . date('Y-m-d') . "')";
  }
  else if ($cmbFilter=="Out")
  {  
   $query_count = "SELECT * FROM `transactions` WHERE `Transaction Type` in ('Withdrawal','Commission') and (`Date` = '" . date('Y-m-d') . "')";
  }
  ...
  {  
   $query_count = "SELECT * FROM `transactions` WHERE (`Date` = '" . date('Y-m-d') . "') and (`Officer` like '%" . $filter . "%' or `Officer` like '%" . $filter2 . "%')";
  }
