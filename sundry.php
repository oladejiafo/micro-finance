...
$sql="SELECT * FROM `sundry` WHERE `ID`='$id'";
...
<div class='row' style="background-color:#394247; width:100%" align="center">
  <font face="Verdana" color="#FFFFFF" style="font-size: 16pt">Sundry Income</font></b>
 </div>

<br>
<div align="center">
   <form  action="sundry.php" method="post">
      Select Criteria to Search with: <select size="1" name="cmbFilter" style="width:120px;height:35px">
...
$quer=mysqli_query($conn,"SELECT `Narration` FROM `narration` where `Type`='$type' order by `Narration`"); 
...
if (!$cmbFilter=="")
{  
   $query_count    = "SELECT * FROM `sundry` WHERE `" . $cmbFilter . "` like '" . $filter . "%' order by `Date` desc";
} else {
   $query_count    = "SELECT * FROM `sundry` order by `Date` desc";
}
...
if (!$cmbFilter=="")
{
   $query="SELECT `ID`,`Date`,`Amount`,`Source`,`Account Number` FROM `sundry` WHERE `" . $cmbFilter . "` like '" . $filter . "%' order by `Date` desc LIMIT $limitvalue, $limit";
} else {
   $query="SELECT `ID`,`Date`,`Amount`,`Source`,`Account Number` FROM `sundry` order by `Date` desc LIMIT $limitvalue, $limit";
}
