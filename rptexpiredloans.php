...
<TABLE width='100%' border='1' cellpadding='1' cellspacing='1' align='center' bordercolor="#ccCCcc" id="table2">
 <?php
   ...
   $resultX = mysqli_query($conn,"SELECT `ID`,`Loan Date`, `Loan Duration` FROM `loan` where `Balance` > 0 and `Loan Status`='Active'"); 
   while(list($idX,$dateX,$durX)=mysqli_fetch_row($resultX)) 
   {	
   $valt="date(`Loan Date`) <'" . date('Y-m-d', strtotime('-' . $durX . 'month')) . "'";
   $result = mysqli_query($conn,"SELECT `ID`,`Account Number`, `Loan Amount`, `Loan Date`, `Loan Duration`, `Payment todate`, `Balance`,`Loan Type` FROM `loan` where ($valt) and `Balance` > 0 and `Loan Status`='Active'"); 
