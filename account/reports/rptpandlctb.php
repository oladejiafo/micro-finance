...
define("EW_REPORT_TABLE_VAR", "pandl", TRUE);
define("EW_REPORT_TABLE_SESSION_GROUP_PER_PAGE", "pandl_grpperpage", TRUE);
define("EW_REPORT_TABLE_SESSION_START_GROUP", "pandl_start", TRUE);
define("EW_REPORT_TABLE_SESSION_SEARCH", "pandl_search", TRUE);
define("EW_REPORT_TABLE_SESSION_CHILD_USER_ID", "pandl_childuserid", TRUE);
define("EW_REPORT_TABLE_SESSION_ORDER_BY", "pandl_orderby", TRUE);
...
$EW_REPORT_TABLE_SQL_SELECT = "SELECT `Type`, `Classification`, Year(`Date`) AS Year__Date, <DistinctColumnFields> FROM " . $EW_REPORT_TABLE_SQL_FROM;
$EW_REPORT_TABLE_SQL_WHERE = "";
$EW_REPORT_TABLE_SQL_GROUPBY = "`Type`, `Classification`, Year(`Date`)";
$EW_REPORT_TABLE_SQL_HAVING = "";
$EW_REPORT_TABLE_SQL_ORDERBY = "`Type` ASC, `Classification` ASC, Year(`Date`)";
...
<?php if (@$sExport == "") { ?>
&nbsp;&nbsp;<a href="rptpandlctb.php?export=html">Printer Friendly</a>
&nbsp;&nbsp;<a href="rptpandlctb.php?export=excel">Export to Excel</a>
&nbsp;&nbsp;<a href="rptpandlctb.php?export=word">Export to Word</a>
<?php } ?>
<br /><br />
<?php if (@$sExport == "") { ?>
</div></td></tr>
...
<!-- Search form (begin) -->
<form name="fpandlcrosstabfilter" id="fpandlcrosstabfilter" action="rptpandlctb.php" class="ewForm" onSubmit="return ewrpt_ValidateExtFilter(this);">
...
<?php if ($bShowFirstHeader) { // Show header ?>
	...
	<!-- Table header -->
	<tr class="ewTableRow">
		<td colspan="2" nowrap><div class="phpreportmaker">&nbsp;</div></td>
...
while ($rsgrp && !$rsgrp->EOF) {
	...
	$sWhere = ewrpt_DetailFilterSQL(EW_REPORT_TABLE_FIRST_GROUP_FIELD, $x_Type, EW_REPORT_DATATYPE_STRING);
	if ($sFilter != "")
...
//	echo "sql: " . $sSql . "<br>";
	...
	while ($rs && !$rs->EOF) {
		...
		// Show group values
		$g_Type = $x_Type;
		if ($x_Type <> "" && $o_Type == $x_Type && !ChkLvlBreak(1)) {
			$g_Type = "&nbsp;";
		} elseif (is_null($x_Type)) {
			$g_Type = EW_REPORT_NULL_LABEL;
		} elseif ($x_Type == "") {
			$g_Type = EW_REPORT_EMPTY_LABEL;
		}
		$g_Classification = $x_Classification;
		if ($x_Classification <> "" && $o_Classification == $x_Classification && !ChkLvlBreak(2)) {
			$g_Classification = "&nbsp;";
...
?>
	...
	<tr>
		<!-- Type -->
		<td class="ewRptGrpField1"><?php $t_Type = $x_Type; $x_Type = $g_Type; ?>
<?php echo ewrpt_ViewValue($x_Type) ?>
<?php $x_Type = $t_Type; ?></td>
		<!-- Classification -->
		<td class="ewRptGrpField2"><?php $t_Classification = $x_Classification; $x_Classification = $g_Classification; ?>
...
<?php
	...
	// Save old group values
	$o_Remark = $x_Type; // Save old group value
	GetGrpRow(2);
	$nGrpCount++;
?>
<?php
...
?>
	...
	<tr>
		<?php $t_Type = $x_Type; $x_Type = $o_Type; ?>
		<td class="ewRptGrpSummary1" colspan="2">Summary for Type: 
<?php echo ewrpt_ViewValue($x_Type) ?>
</td>
		<?php $x_Type = $t_Type; ?>
<!-- Dynamic columns begin -->
...
<form action="rptpandlctb.php" name="ewpagerform" id="ewpagerform" class="ewForm">
...
<!--first page button-->
	...
	<td><a href="rptpandlctb.php?start=<?php echo $Pager->FirstButton->Start ?>"><img src="phprptimages/first.gif" alt="First" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
	<td><img src="phprptimages/firstdisab.gif" alt="First" width="16" height="16" border="0"></td>
	<?php } ?>
<!--previous page button-->
	<?php if ($Pager->PrevButton->Enabled) { ?>
	<td><a href="rptpandlctb.php?start=<?php echo $Pager->PrevButton->Start ?>"><img src="phprptimages/prev.gif" alt="Previous" width="16" height="16" border="0"></a></td>
	<?php } else { ?>
...
<!--next page button-->
	...
	<td><a href="rptpandlctb.php?start=<?php echo $Pager->NextButton->Start ?>"><img src="phprptimages/next.gif" alt="Next" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
	<td><img src="phprptimages/nextdisab.gif" alt="Next" width="16" height="16" border="0"></td>
	<?php } ?>
<!--last page button-->
	<?php if ($Pager->LastButton->Enabled) { ?>
	<td><a href="rptpandlctb.php?start=<?php echo $Pager->LastButton->Start ?>"><img src="phprptimages/last.gif" alt="Last" width="16" height="16" border="0"></a></td>	
	<?php } else { ?>
...
function GetRow($opt) {
	...
	if (!$rs->EOF) {
		$GLOBALS['x_Classification'] = $rs->fields('Classification');
		$cntval = count($val);
...
function ChkLvlBreak($lvl) {
	switch ($lvl) {
		...
		case 2:
			...
			($GLOBALS["x_Classification"] <> $GLOBALS["o_Classification"]) || ChkLvlBreak(1); // Recurse upper level
	}
}
...
function SetupPopup() {
	...
	if (@$_GET["Year__Date"] <> "") {
		...
		if (is_numeric($GLOBALS["x_Year__Date"])) {
			$_SESSION["sel_pandl_Year__Date"] = $GLOBALS["x_Year__Date"];
			ResetPager();
		}
	}
	$GLOBALS["sel_Year__Date"] = @$_SESSION["sel_pandl_Year__Date"];
}
...
function getSort()
{
	...
	if (strlen(@$_GET["cmd"]) > 0) {
		...
		if ($sCmd == "resetsort") {
			...
			$_SESSION["sort_pandl_Type"] = "";
			$_SESSION["sort_pandl_Classification"] = "";
		}
